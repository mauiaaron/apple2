...
void show_regs() {
    ...
    if (cpu65_current.f & C_Flag) second_buf[num_buffer_lines][0]='C';
    if (cpu65_current.f & X_Flag) second_buf[num_buffer_lines][1]='X';
    if (cpu65_current.f & I_Flag) second_buf[num_buffer_lines][2]='I';
    if (cpu65_current.f & V_Flag) second_buf[num_buffer_lines][3]='V';
    if (cpu65_current.f & B_Flag) second_buf[num_buffer_lines][4]='B';
    if (cpu65_current.f & D_Flag) second_buf[num_buffer_lines][5]='D';
    if (cpu65_current.f & Z_Flag) second_buf[num_buffer_lines][6]='Z';
    if (cpu65_current.f & N_Flag) second_buf[num_buffer_lines][7]='N';
...
void c_do_step(int step_count) {
	...
	else cpu65_step();			/* step one instruction */
